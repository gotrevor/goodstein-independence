...
## Where it stands
**(lap-149 FRESH-MIND REVIEW — CURRENT read.)** Build green **1326**; re-verified in-kernel the headline +
the whole crux-2 chain (`false_of_ZDerivesEmpty`, `ZDerivesEmptyR_descent_step`, `descent_step_K_noncrit_recurse`)
are all `[propext, sorryAx, choice, Quot.sound]` — **0 math axioms**, no drift. Single open obligation = crux-2
(IΣ₁-internal Gentzen termination), now reduced to **four disclosed live `sorryAx` leaves**, none generational,
none needing `red`: (1) tag-3 freshFlag `Crux2Blueprint:2974`; (2) tag-4 K-recursion :2934; (3) tag-5/6 axMajor
cut-partner :3002; (4) (A) gDef :3125. Leaves (2)+(3) ARE the genuine crux — the **general `Γ→⊥` cut-reduction
by strong induction on derivation CODE** (Buchholz Thm 2.1 / §14.253-254; NOT `iord`-recursion, PRWO/Gödel-barred);
(4) is the parallel Σ₁-definability crux. **This lap's call: KEEP the existence-form pivot; close the LAST
tractable leaf (tag-3 freshFlag) via the focused `zFreshNext` tag-3→`freshFlag` strengthening (mirror tag-1 I∀,
exact shape of the proven lap-146 `zIndWff` ripple) → tag-3 repMajor sorry-free; THEN attack the general
code-recursion crux head-on instead of deferring to ever-smaller leaves.** See `DIRECTION.md` CURRENT DIRECTIVE
(lap-149), `PENDING_WORK.md`.
