...
theorem tp_red_isymRep_of_zTag_4 {d : V} (hZ : ZDerivation d) (htag : zTag d = 4) :
  ...
  · rcases zDerivation_iff.mp hZ with ⟨s', heq, _⟩ | ⟨s', a, p, d0, heq, _, _, _⟩ |
      ...
      ⟨s', p, k, heq, _, _⟩ | ⟨s', p, heq, _, _⟩ | ⟨s', C, heq, _⟩ | ⟨s', heq, _⟩
    ...
    · exact absurd (heq ▸ htag) (by rw [zTag_zAxBot]; simp)
...
theorem red_rep_of_tp_isymRep {v : V} (hZ : ZDerivation v) (htp : tp v = isymRep)
  ...
  · exact ⟨by rw [red_zAxBot], by rw [red_zAxBot, tp_zAxBot]⟩
...
theorem ZDerivation_red_zK_nonRep {s r ds : V}
  ...
  · exact absurd (by rw [heq]; exact tp_zAxBot s') htp
  ...
  have key : ∀ d : V, ZDerivation d → (ZRegular d → ZDerivation (red d)) := by
    ...
    · intro C hC d hphi hreg
      rcases hphi with ⟨s, rfl, hin⟩ | ⟨s, a, p, d0, rfl, hd0, hsucc, hwff⟩ |
        ...
        ⟨s, p, k, rfl, hp, hin⟩ | ⟨s, p, rfl, hp, hin, hin2⟩ | ⟨s, C, rfl, hin⟩ | ⟨s, rfl, hin⟩
      · -- zAtom: red = identity
      ...
      · -- zAx1: red = identity
        rw [red_zAx1]; exact zDerivation_iff.mpr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr
          (Or.inr (Or.inr (Or.inl ⟨s, C, rfl, hin⟩))))))))
      · -- zAxBot: red = identity
        rw [red_zAxBot]; exact zDerivation_iff.mpr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr
          (Or.inr (Or.inr (Or.inr ⟨s, rfl, hin⟩))))))))
  exact key
  ...
  · -- K/cut (tag 4): dispatch on the `permIdx` criticality sentinel.
    ...
    · -- the genuine K-rule node `zK s r ds`
      ...
      · -- NON-critical: `red (zK s r ds) = K^r(i/red dᵢ)` (replace, 5.2.2), `i = permIdx`. Dispatch on the
        ...
        · -- zAxBot (tag 8): `red dᵢ = dᵢ` (`red_zAxBot`), so the whole node is a `red`-FIXPOINT (LEFT).
          exact Or.inl (red_zK_fixpoint_of_zAxBot_selected hds hcrit heq)
...
lemma genReduct_chain_noRedex_anySucc {s r ds j0 : V}
  ...
  -- §14.254b — the unified NON-Rep producer closer (lap 165), instantiated with the anySucc closers and the
  -- residual `residual` (the R1/R2 escapes, VACUOUS at `Γ = ∅`). THE keystone least-number recursion.
  have cnrp : ∀ m, m ≤ j0 → m < lh ds →
  ...
  have tryProducerClose : ∀ m, m ≤ j0 → m < lh ds → zTag (znth ds m) ≠ 0 → zTag (znth ds m) ≠ 7 →
    ...
    · -- tag 5 (zAxAll): NON-Rep producer → the unified closer (lap 165 keystone recursion).
      exact cnrp m hmj0 hmlt (Or.inl (by rw [h'']; simp))
    · -- tag 6 (zAxNeg): NON-Rep producer → the unified closer.
      exact cnrp m hmj0 hmlt (Or.inr (Or.inl (by rw [h'']; simp)))
    · exact absurd (show zTag (znth ds m) = 7 by rw [h'']; simp) hmne7          -- tag 7: PHANTOM (non-leaf)
    · -- tag 8 (zAxBot): NON-Rep producer → the unified closer.
      exact cnrp m hmj0 hmlt (Or.inr (Or.inr (by rw [h'']; simp)))
  ...
  · -- tag 6 (zAxNeg): the major IS a NON-Rep producer → the unified closer (lap 165 keystone recursion).
    exact cnrp jstar hjle hjlt (Or.inr (Or.inl (by rw [h]; simp)))
  · -- tag 7 (zAx1): leaf, like tag 0.
    ...
    · rcases hjstar with hC | hbot
      ...
      · exact exFalsoClose (hbot ▸ hDin)
    · exact absurd (heq ▸ hjstar) (hmin i' hi')
  · -- tag 8 (zAxBot): ex-falso major. `⊥∈Γ_jstar` threads to `⊥∈Γ` (→ exFalsoClose) or an earlier
    -- `⊥`-exit `i' < jstar`, contradicting jstar minimality.
    have hbotin : inAnt (^⊥ : V) (chainAnt ds jstar) := by
      have hinv := zDerivation_zAxBot_inv (h ▸ hmemZ)
      simpa only [chainAnt, h, fstIdx_zAxBot] using hinv
...
lemma genReduct_chain_noRedex {s r ds j0 : V}
  ...
  -- `certReplace_of_premise_cert`. This is the lap-158 spike's `noRedex_producer_closes`, ported. It closes
  -- the IRREDUCIBLE {3,4}-producer core of `axMajorResidual` (the rest of `axMajorResidual` is now only the
  -- general-`C` leaf/escape closes, which still derive `Γ→⊥`).
  have repProducerClose : ∀ m, m ≤ j0 → m < lh ds →
      (zTag (znth ds m) = 3 ∨ zTag (znth ds m) = 4) → GenReductCert (zK s r ds) :=
    fun m hmj0 hmlt htagm =>
      Or.inl (certReplace_of_premise_cert hZ hreg hfresh hseqant hj0 hthread0 hrank0 (Or.inr hbot0)
        hmlt hmj0 (genReduct_anySucc (hmem m hmlt) (ZRegular_zK_premise hds hreg hmlt)
          (ZFresh_zK_premise hds hfresh hmlt) (ZSeqAnt_zK_premise hds hseqant hmlt) htagm))
      ...
      ⟨by rw [idg_zAxNeg]; exact zero_le,
       by rw [iotil_zAxNeg, iotil_zK s r ds hds]
          exact finHead_iotil_lt_iseqNaddIdg hjw
            ...
            hXq hNF,
       by rw [iotil_zAxNeg, ← hXq]; exact hNF jw⟩⟩
  -- §14.254b — the unified NON-Rep producer closer (lap 165), instantiated with the local closers and the
  -- residual `axMajorResidual` (the R1/R2 escapes, VACUOUS at `Γ = ∅`). THE keystone least-number recursion.
  have cnrp : ∀ m, m ≤ j0 → m < lh ds →
      (zTag (znth ds m) = 5 ∨ zTag (znth ds m) = 6 ∨ zTag (znth ds m) = 8) →
      GenReductCert (zK s r ds) :=
  ...
  -- CONSTRUCTOR. tag 6 (zAxNeg) → `closeZAxNeg`. tag 5 (zAxAll) → the rank-CLIMB
  -- (`climb_to_rep_producer`): its active `^∀ p''` threads UP one rank, so it can only bottom out at a
  ...
  · -- tag 5 (zAxAll): L-axiom `Ax^{k'}_{∀p'}` major (`red`-FIXPOINT). The ⊥-exit + `zAxAllSuccWff` force
    ...
    · by_cases h0 : π₁ (tp (znth ds m)) = 0
      ...
      · exact tryProducerClose m (le_of_lt (lt_of_lt_of_le hmjs hjle)) (lt_trans hmjs hjlt) hm0 hm7 h0
  · -- tag 6 (zAxNeg): the major IS a NON-Rep producer → the unified closer (lap 165 keystone recursion).
    -- (The least `{5,6,8}`-producer the closer selects subsumes this `jstar` dispatch with `n`-leastness.)
    exact cnrp jstar hjle hjlt (Or.inr (Or.inl (by rw [h]; simp)))
  · -- tag 7 (zAx1): a leaf (§5 logical axiom, like zAtom); `⊥ ∈ Γ`. Same trivial-axiom collapse. PROVEN.
    ...
    · exact hh
    · exact absurd heq.symm (hmin i' hi')
  · -- tag 8 (zAxBot): ex-falso leaf; `⊥ ∈ Γ_jstar` directly. Same `leafClose` collapse as tag 0/7.
    refine leafClose ?_
    have hin : inAnt (^⊥ : V) (chainAnt ds jstar) := by
      simpa only [chainAnt, h, fstIdx_zAxBot] using zDerivation_zAxBot_inv (h ▸ hmemZ)
    rcases hthread0 jstar hjle (^⊥) hin with hh | ⟨i', hi', heq⟩
    · exact hh
    · exact absurd heq.symm (hmin i' hi')
