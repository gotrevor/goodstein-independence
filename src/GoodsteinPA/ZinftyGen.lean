...
`SyntacticFormula L` (full FO with total de-Morgan negation `∼`, finite `complexity : … → ℕ`,
...
Sequents are **`Finset (SyntacticFormula L)`**, matching Towsner (his `Γ` is a finite *set*).
...
/- Generic ambient language with the M5/M4 instance bundle (GenericZinftyProbe.lean):
   order-ring symbols (numerals), the ℕ-model (LitTrue), decidable symbols (Finset sequents).
   Re-instantiated at L (existing users) and LX = L+X (Boundedness). -/
variable {L : Language} [Language.ORing L] [Structure L ℕ]
  [(k : ℕ) → DecidableEq (L.Func k)] [(k : ℕ) → DecidableEq (L.Rel k)]

/-- The closed formulas of `L` (full first-order syntax, with total de-Morgan negation `∼`
and finite `complexity`). -/
abbrev Form (L : Language) := SyntacticFormula L

/-- The `n`-th numeral of `L` as a closed term, ready for substitution `φ/[nm n]`. -/
...
abbrev Seq (L : Language) := Finset (Form L)
...
def signedLit : Bool → {k : ℕ} → (L).Rel k → (Fin k → Semiterm L ℕ 0) → Form L
...
theorem litTrue_or_neg (φ : Form L) : LitTrue φ ∨ LitTrue (∼φ) := by
  rw [litTrue_neg]; exact em _

/-- **Value-congruence of literal truth.** A relation's truth in the ℕ-model depends only on the
...
@[simp] theorem neg_lit (b : Bool) {k} (r : (L).Rel k) (v) :
...
theorem litTrue_flip (b : Bool) {k} (r : (L).Rel k) (v) :
    LitTrue (signedLit (!b) r v) ↔ ¬ LitTrue (signedLit b r v) := by
  rw [← neg_lit]; exact litTrue_neg _

/-- A signed literal is never `⊤`. -/
@[simp] theorem lit_ne_verum (b : Bool) {k} (r : (L).Rel k) (v) :
    signedLit b r v ≠ (⊤ : Form L) := by cases b <;> simp [signedLit]

/-- A signed literal is never `⊥`. -/
@[simp] theorem lit_ne_falsum (b : Bool) {k} (r : (L).Rel k) (v) :
    signedLit b r v ≠ (⊥ : Form L) := by cases b <;> simp [signedLit]

/-- **The `Z_∞` calculus** over real `L` syntax. The `allω` (ω-rule) constructor stores one
sub-derivation per numeral `n`: from `insert (φ/[nm n]) Γ` for every `n`, conclude
`insert (∀⁰ φ) Γ`. -/
inductive Deriv : Seq L → Type
  ...
  | verumR {Γ : Seq L} (h : (⊤ : Form L) ∈ Γ) : Deriv Γ
  | weak {Δ Γ : Seq L} (d : Deriv Δ) (h : Δ ⊆ Γ) : Deriv Γ
  | andI {Γ : Seq L} (φ ψ : Form L) (dφ : Deriv (insert φ Γ)) (dψ : Deriv (insert ψ Γ)) :
      Deriv (insert (φ ⋏ ψ) Γ)
  | orI {Γ : Seq L} (φ ψ : Form L) (d : Deriv (insert φ (insert ψ Γ))) : Deriv (insert (φ ⋎ ψ) Γ)
  | allω {Γ : Seq L} (φ : SyntacticSemiformula L 1)
      (d : (n : ℕ) → Deriv (insert (φ/[nm n]) Γ)) : Deriv (insert (∀⁰ φ) Γ)
  | exI {Γ : Seq L} (φ : SyntacticSemiformula L 1) (n : ℕ)
      (d : Deriv (insert (φ/[nm n]) Γ)) : Deriv (insert (∃⁰ φ) Γ)
  | cut {Γ : Seq L} (φ : Form L) (d₁ : Deriv (insert φ Γ)) (d₂ : Deriv (insert (∼φ) Γ)) : Deriv Γ
...
noncomputable def o : {Γ : Seq L} → Deriv Γ → Ordinal.{0}
  | _, axL _ _ _ _ => 0
  | _, axLv _ _ _ _ _ _ => 0
...
derivation has `cr = 0`. -/
noncomputable def cr : {Γ : Seq L} → Deriv Γ → ℕ∞
  | _, axL _ _ _ _ => 0
  | _, axLv _ _ _ _ _ _ => 0
  | _, axTrue _ _ _ _ _ => 0
  | _, verumR _ => 0
...
def Provable (α : Ordinal.{0}) (c : ℕ) (Γ : Seq L) : Prop :=
  ∃ d : Deriv Γ, o d ≤ α ∧ cr d ≤ (c : ℕ∞)

/-- The ω-rule bound strictly dominates each premise bound. -/
theorem o_allω_gt {Γ : Seq L} (φ : SyntacticSemiformula L 1)
...
theorem Provable.mono {α β : Ordinal.{0}} {c c' : ℕ} (hα : α ≤ β) (hc : c ≤ c') {Γ : Seq L} :
...
theorem Provable.weakening {α : Ordinal.{0}} {c : ℕ} {Γ Δ : Seq L} (h : Γ ⊆ Δ) :
...
theorem Provable.cast {α : Ordinal.{0}} {c : ℕ} {Γ Δ : Seq L} (e : Γ = Δ) :
    Provable α c Γ → Provable α c Δ := fun h => e ▸ h

/-- Identity axiom: `rel r v` and `nrel r v` together close at bound `0`, cut rank `0`. -/
theorem Provable.axL {Γ : Seq L} {k} (r : (L).Rel k) (v)
    (hp : Semiformula.rel r v ∈ Γ) (hn : Semiformula.nrel r v ∈ Γ) : Provable 0 0 Γ :=
  ⟨Deriv.axL r v hp hn, by simp [Deriv.o], by simp [Deriv.cr]⟩

/-- **Value-congruent literal axiom** (Buchholz `AX(Z∞)`, lecture notes p.27): a complementary literal
pair `{rel r v, nrel r v'}` whose argument vectors have equal standard values closes any sequent
...
theorem Provable.axTrue {Γ : Seq L} {k} (b : Bool) (r : (L).Rel k) (v)
...
theorem Provable.verumR {Γ : Seq L} (h : (⊤ : Form L) ∈ Γ) : Provable 0 0 Γ :=
  ⟨Deriv.verumR h, by simp [Deriv.o], by simp [Deriv.cr]⟩

/-- Predicate-level `∧`-introduction. -/
theorem Provable.andI {α β : Ordinal.{0}} {c : ℕ} {Γ : Seq L} (φ ψ : Form L)
...
/-- Predicate-level `∨`-introduction. -/
theorem Provable.orI {α : Ordinal.{0}} {c : ℕ} {Γ : Seq L} (φ ψ : Form L)
...
term — deferred to M4.) -/
theorem Provable.exI {α : Ordinal.{0}} {c : ℕ} {Γ : Seq L} (φ : SyntacticSemiformula L 1)
...
theorem Provable.allω {β : ℕ → Ordinal.{0}} {c : ℕ} {Γ : Seq L}
    (φ : SyntacticSemiformula L 1) (h : ∀ n, Provable (β n) c (insert (φ/[nm n]) Γ)) :
...
theorem Provable.contr {α : Ordinal.{0}} {c : ℕ} {Γ : Seq L} (φ : Form L)
...
theorem Provable.cut {α β : Ordinal.{0}} {c : ℕ} {Γ : Seq L} (χ : Form L)
...
private theorem invPush (a : Form L) (s : Seq L) :
...
private theorem invPull {a : Form L} (h : a ≠ (φ ⋎ ψ)) (s : Seq L) :
...
theorem orInvAux {c : ℕ} : ∀ {Γ : Seq L} (d : Deriv Γ), cr d ≤ (c : ℕ∞) → (φ ⋎ ψ) ∈ Γ →
  ...
  | @axLv Γ k r v v' hval hp hn =>
    ...
    have hn' : Semiformula.nrel r v' ∈ Γ.erase (φ ⋎ ψ) :=
      Finset.mem_erase.mpr ⟨by intro h; simp [Vee.vee] at h, hn⟩
    simp only [Deriv.o]
    exact (Provable.axLv r v v' hval (Finset.mem_insert_of_mem (Finset.mem_insert_of_mem hr))
      (Finset.mem_insert_of_mem (Finset.mem_insert_of_mem hn'))).mono le_rfl (Nat.zero_le c)
...
theorem Provable.orInv {α : Ordinal.{0}} {c : ℕ} {Γ : Seq L} (hmem : (φ ⋎ ψ) ∈ Γ)
...
variable {χ : SyntacticSemiformula L 1}

/-- Reorder helper (single insert): invert under `insert a`, push it outside. -/
private theorem invPush1 (b a : Form L) (e : Form L) (s : Seq L) :
...
private theorem invPull1 (b : Form L) {a e : Form L} (h : a ≠ e) (s : Seq L) :
...
theorem allInvAux {c : ℕ} (n : ℕ) : ∀ {Γ : Seq L} (d : Deriv Γ), cr d ≤ (c : ℕ∞) →
  ...
  | @axLv Γ k r v v' hval hp hn =>
    ...
    have hn' : Semiformula.nrel r v' ∈ Γ.erase (∀⁰ χ) :=
      Finset.mem_erase.mpr ⟨Semiformula.ne_of_ne_complexity (by simp), hn⟩
    simp only [Deriv.o]
    exact (Provable.axLv r v v' hval (Finset.mem_insert_of_mem hr)
      (Finset.mem_insert_of_mem hn')).mono le_rfl (Nat.zero_le c)
  | @axTrue Γ k b r v htrue hmem =>
    intro _ _
...
theorem Provable.allInv {α : Ordinal.{0}} {c : ℕ} {Γ : Seq L} (hmem : (∀⁰ χ) ∈ Γ) (n : ℕ)
...
theorem andInvAux {c : ℕ} : ∀ {Γ : Seq L} (d : Deriv Γ), cr d ≤ (c : ℕ∞) → (φ ⋏ ψ) ∈ Γ →
  ...
  | @axLv Γ k r v v' hval hp hn =>
    ...
    exact ⟨(Provable.axLv r v v' hval (Finset.mem_insert_of_mem hr)
          (Finset.mem_insert_of_mem hn')).mono le_rfl (Nat.zero_le c),
      (Provable.axLv r v v' hval (Finset.mem_insert_of_mem hr)
          (Finset.mem_insert_of_mem hn')).mono le_rfl (Nat.zero_le c)⟩
  | @axTrue Γ k b r v htrue hmem =>
    intro _ _
    have hl : signedLit b r v ∈ Γ.erase (φ ⋏ ψ) :=
...
theorem Provable.andInvL {α : Ordinal.{0}} {c : ℕ} {Γ : Seq L} (hmem : (φ ⋏ ψ) ∈ Γ)
...
theorem Provable.andInvR {α : Ordinal.{0}} {c : ℕ} {Γ : Seq L} (hmem : (φ ⋏ ψ) ∈ Γ)
...
private theorem frame_in (a e : Form L) (s t : Seq L) :
...
private theorem frame_out {a e : Form L} (hne : a ≠ e) (s t : Seq L) :
    insert a (s.erase e ∪ t) ⊆ (insert a s).erase e ∪ t := by
  intro x hx
  simp only [Finset.mem_union, Finset.mem_erase, Finset.mem_insert] at hx ⊢
...
theorem Provable.cutReduceConj {a b : Form L} {c : ℕ} {α β : Ordinal.{0}} {Γ : Seq L}
...
theorem Provable.cutReduceDisj {a b : Form L} {c : ℕ} {α β : Ordinal.{0}} {Γ : Seq L}
    (ha : (a.complexity + 1 : ℕ∞) ≤ c) (hb : (b.complexity + 1 : ℕ∞) ≤ c)
    (hC : Provable α c (insert (a ⋎ b) Γ)) (hNC : Provable β c (insert (∼a ⋏ ∼b) Γ)) :
    Provable (max α β + 1 + 1) c Γ := by
...
∃-side derivation `d`. -/
theorem Provable.cutReduceAllAux {φ : SyntacticSemiformula L 1} {c : ℕ} {α : Ordinal.{0}}
    {Γ : Seq L} (hφc : (φ.complexity + 1 : ℕ∞) ≤ c)
    (fam : ∀ n, Provable α c (insert (φ/[nm n]) Γ)) :
    ∀ {Δ : Seq L} (d : Deriv Δ), cr d ≤ (c : ℕ∞) → (∃⁰ ∼φ) ∈ Δ →
  ...
  | @axLv Δ k r v v' hval hp hn =>
    intro _ _
    simp only [Deriv.o]
    refine (Provable.axLv r v v' hval ?_ ?_).mono zero_le (Nat.zero_le c)
    · exact Finset.mem_union_left _ (Finset.mem_erase.mpr ⟨Semiformula.ne_of_ne_complexity (by simp), hp⟩)
    · exact Finset.mem_union_left _ (Finset.mem_erase.mpr ⟨Semiformula.ne_of_ne_complexity (by simp), hn⟩)
...
theorem Provable.cutReduceAll {φ : SyntacticSemiformula L 1} {c : ℕ} {α β : Ordinal.{0}}
    {Γ : Seq L} (hφc : (φ.complexity + 1 : ℕ∞) ≤ c)
...
theorem Provable.removeFalseLitAux (b₀ : Bool) {k₀} (r₀ : (L).Rel k₀) (v₀)
    (hL : ¬ LitTrue (signedLit b₀ r₀ v₀)) :
    ∀ {Δ : Seq L} (d : Deriv Δ), cr d ≤ (0 : ℕ∞) →
      signedLit b₀ r₀ v₀ ∈ Δ → Provable (o d) 0 (Δ.erase (signedLit b₀ r₀ v₀)) := by
  set Lit : Form L := signedLit b₀ r₀ v₀ with hLdef
  have hLne : ∀ (g : Form L), g.complexity ≠ 0 → g ≠ Lit := by
  ...
  | @axL Δ k r v hp hn =>
    ...
    · -- `Lit = rel r v` (false) ⟹ `nrel r v = ∼(rel r v)` is true ⟹ close by `axTrue false`.
    ...
    · by_cases h2 : Lit = Semiformula.nrel r v
      · -- `Lit = nrel r v` (false) ⟹ `rel r v` is true ⟹ close by `axTrue true`.
  ...
  | @axLv Δ k r v v' hval hp hn =>
    intro _ _; simp only [Deriv.o]
    by_cases h1 : Lit = Semiformula.rel r v
    · -- `Lit = rel r v` (false) ⟹ by value-congruence `rel r v'` false ⟹ `nrel r v'` true.
      have htn : LitTrue (signedLit false r v') := by
        show LitTrue (Semiformula.nrel r v')
        rw [← Semiformula.neg_rel, litTrue_neg]
        exact (litTrue_rel_congr r v v' hval).not.mp (h1 ▸ hL)
      exact Provable.axTrue false r v' htn
        (Finset.mem_erase.mpr ⟨by rw [h1]; simp [signedLit], hn⟩)
    · by_cases h2 : Lit = Semiformula.nrel r v'
      · -- `Lit = nrel r v'` (false) ⟹ `rel r v'` true ⟹ by value-congruence `rel r v` true.
        have htr : LitTrue (signedLit true r v) := by
          show LitTrue (Semiformula.rel r v)
          refine (litTrue_rel_congr r v v' hval).mpr ?_
          by_contra hc
          exact (h2 ▸ hL) (by rw [← Semiformula.neg_rel, litTrue_neg]; exact hc)
        exact Provable.axTrue true r v htr
          (Finset.mem_erase.mpr ⟨by rw [h2]; simp [signedLit], hp⟩)
      · exact Provable.axLv r v v' hval (Finset.mem_erase.mpr ⟨fun e => h1 e.symm, hp⟩)
          (Finset.mem_erase.mpr ⟨fun e => h2 e.symm, hn⟩)
  | @axTrue Δ k b r v htrue hmem =>
    intro _ _; simp only [Deriv.o]
    -- the true witness `signedLit b r v ≠ Lit` (false), so it survives the erase.
  ...
  | @andI Γ₀ χ₀ χ₁ d₀ d₁ ih₀ ih₁ =>
    ...
    have P0 : Provable (o d₀) 0 (insert χ₀ (Γ₀.erase Lit)) :=
      (ih₀ (le_trans (le_max_left _ _) hcr) (Finset.mem_insert_of_mem hmem0)).weakening (by
        intro x hx; simp only [Finset.mem_insert, Finset.mem_erase] at hx ⊢; tauto)
    have P1 : Provable (o d₁) 0 (insert χ₁ (Γ₀.erase Lit)) :=
  ...
  | @orI Γ₀ χ₀ χ₁ d' ih =>
    ...
    have hhead : (χ₀ ⋎ χ₁) ≠ Lit := hLne _ (by simp)
    have hmem0 : Lit ∈ Γ₀ := (Finset.mem_insert.mp hmem).resolve_left fun e => hhead e.symm
    have P : Provable (o d') 0 (insert χ₀ (insert χ₁ (Γ₀.erase Lit))) :=
      (ih hcr (Finset.mem_insert_of_mem (Finset.mem_insert_of_mem hmem0))).weakening (by
        intro x hx; simp only [Finset.mem_insert, Finset.mem_erase] at hx ⊢; tauto)
  ...
  | @allω Γ₀ χ' d' ih =>
    ...
    have hmem0 : Lit ∈ Γ₀ := (Finset.mem_insert.mp hmem).resolve_left fun e => hhead e.symm
    have key : ∀ n, Provable (o (d' n)) 0 (insert (χ'/[nm n]) (Γ₀.erase Lit)) := fun n =>
  ...
  | @exI Γ₀ χ' n d' ih =>
    ...
    have hhead : (∃⁰ χ') ≠ Lit := hLne _ (by simp)
    have hmem0 : Lit ∈ Γ₀ := (Finset.mem_insert.mp hmem).resolve_left fun e => hhead e.symm
    have P : Provable (o d') 0 (insert (χ'/[nm n]) (Γ₀.erase Lit)) :=
      (ih hcr (Finset.mem_insert_of_mem hmem0)).weakening (by
        intro x hx; simp only [Finset.mem_insert, Finset.mem_erase] at hx ⊢; tauto)
...
theorem Provable.atomCutAux {k} (r : (L).Rel k) (v) {B : Ordinal.{0}} {Γ : Seq L}
    (hNC : Provable B 0 (insert (Semiformula.nrel r v) Γ)) :
    ∀ {Δ : Seq L} (d : Deriv Δ), cr d ≤ (0 : ℕ∞) → (Semiformula.rel r v) ∈ Δ →
  ...
  | @axL Δ k' r' v' hp hn =>
    intro _ _
    simp only [Deriv.o]
    have hnn : (Semiformula.nrel r' v' : Form L) ∈ Δ.erase (Semiformula.rel r v) :=
      Finset.mem_erase.mpr ⟨by intro h; exact absurd h (by simp), hn⟩
    by_cases hrel : (Semiformula.rel r' v' : Form L) = Semiformula.rel r v
    · -- the clash's positive member IS the cut atom ⇒ `nrel r v ∈ Γ`-part, use `hNC`
      have hnrv : (Semiformula.nrel r' v' : Form L) = Semiformula.nrel r v := by
    ...
    · -- clash avoids the cut atom ⇒ it survives the erase, close by `axL`
      have hpp : (Semiformula.rel r' v' : Form L) ∈ Δ.erase (Semiformula.rel r v) :=
  ...
  | @axLv Δ k' r₀ va vb hval hp hn =>
    intro _ _
    simp only [Deriv.o]
    have hnn : (Semiformula.nrel r₀ vb : Form L) ∈ Δ.erase (Semiformula.rel r v) :=
      Finset.mem_erase.mpr ⟨by intro h; exact absurd h (by simp), hn⟩
    by_cases hrel : (Semiformula.rel r₀ va : Form L) = Semiformula.rel r v
    · -- the value-congruent pair's positive member IS the cut atom; split on its truth.
      rcases litTrue_or_neg (Semiformula.rel r v) with htt | htf
      · -- `rel r v` TRUE ⟹ `nrel r v` is a removable false literal on the `hNC` side.
        have hfalse : ¬ LitTrue (signedLit false r v) := by
          rw [← litTrue_flip false r v]; simpa [signedLit] using htt
        ...
        · intro x hx
          have hxΓ : x ∈ Γ := by
            ...
            · exact absurd (show (Semiformula.nrel r v : Form L) = signedLit false r v by simp [signedLit]) h2
            · exact h3
          exact Finset.mem_union_right _ hxΓ
        · exact le_trans hoN (le_trans le_self_add (le_of_lt (lt_add_of_pos_right _ one_pos)))
      · -- `rel r v` FALSE ⟹ by value-congruence `rel r₀ vb` false ⟹ `nrel r₀ vb` TRUE.
        have hva : ¬ LitTrue (Semiformula.rel r₀ va) := by rw [hrel]; exact (litTrue_neg _).mp htf
        have htn : LitTrue (signedLit false r₀ vb) := by
          show LitTrue (Semiformula.nrel r₀ vb)
          rw [← Semiformula.neg_rel, litTrue_neg]
          exact (litTrue_rel_congr r₀ va vb hval).not.mp hva
        exact (Provable.axTrue false r₀ vb htn (Finset.mem_union_left _ hnn)).mono zero_le le_rfl
    · -- the pair's positive member survives the erase ⟹ close by `axLv`.
      have hpp : (Semiformula.rel r₀ va : Form L) ∈ Δ.erase (Semiformula.rel r v) :=
        Finset.mem_erase.mpr ⟨hrel, hp⟩
      exact (Provable.axLv r₀ va vb hval (Finset.mem_union_left _ hpp)
        (Finset.mem_union_left _ hnn)).mono zero_le le_rfl
  | @axTrue Δ k' b' r' v' htrue' hmem' =>
    intro _ _
    simp only [Deriv.o]
    by_cases heq : (signedLit b' r' v' : Form L) = Semiformula.rel r v
...
theorem Provable.atomCut {k} (r : (L).Rel k) (v) {A B : Ordinal.{0}} {Γ : Seq L}
...
constant-`⊥` case). -/
theorem Provable.removeFalsumAux : ∀ {Δ : Seq L} (d : Deriv Δ), cr d ≤ (0 : ℕ∞) →
    (⊥ : Form L) ∈ Δ → Provable (o d) 0 (Δ.erase ⊥) := by
  intro Δ d
  induction d with
  | @axL Δ k r v hp hn =>
    intro _ _; simp only [Deriv.o]
  ...
  | @axLv Δ k r v v' hval hp hn =>
    intro _ _; simp only [Deriv.o]
    exact Provable.axLv r v v' hval (Finset.mem_erase.mpr ⟨by simp, hp⟩)
      (Finset.mem_erase.mpr ⟨by simp, hn⟩)
  | @axTrue Δ k b r v htrue hmem =>
    intro _ _; simp only [Deriv.o]
  ...
  | @weak Δ' Δ d' hsub ih =>
    intro hcr hmem; simp only [Deriv.cr] at hcr; simp only [Deriv.o]
    by_cases hd : (⊥ : Form L) ∈ Δ'
    · exact (ih hcr hd).weakening (Finset.erase_subset_erase _ hsub)
    · refine (show Provable (o d') 0 Δ' from ⟨d', le_rfl, hcr⟩).weakening ?_
      intro x hx; exact Finset.mem_erase.mpr ⟨fun e => hd (e ▸ hx), hsub hx⟩
  | @andI Γ₀ χ₀ χ₁ d₀ d₁ ih₀ ih₁ =>
    intro hcr hmem; simp only [Deriv.cr] at hcr; simp only [Deriv.o]
    have hhead : (χ₀ ⋏ χ₁) ≠ (⊥ : Form L) := by simp [Wedge.wedge]
    have hmem0 : (⊥ : Form L) ∈ Γ₀ := (Finset.mem_insert.mp hmem).resolve_left fun e => hhead e.symm
  ...
  | @orI Γ₀ χ₀ χ₁ d' ih =>
    intro hcr hmem; simp only [Deriv.cr] at hcr; simp only [Deriv.o]
    have hhead : (χ₀ ⋎ χ₁) ≠ (⊥ : Form L) := by simp [Vee.vee]
    have hmem0 : (⊥ : Form L) ∈ Γ₀ := (Finset.mem_insert.mp hmem).resolve_left fun e => hhead e.symm
  ...
  | @allω Γ₀ χ' d' ih =>
    intro hcr hmem; simp only [Deriv.cr] at hcr; simp only [Deriv.o]
    have hhead : (∀⁰ χ') ≠ (⊥ : Form L) := by simp [UnivQuantifier.all]
    have hmem0 : (⊥ : Form L) ∈ Γ₀ := (Finset.mem_insert.mp hmem).resolve_left fun e => hhead e.symm
  ...
  | @exI Γ₀ χ' n d' ih =>
    intro hcr hmem; simp only [Deriv.cr] at hcr; simp only [Deriv.o]
    have hhead : (∃⁰ χ') ≠ (⊥ : Form L) := by simp [ExsQuantifier.exs]
    have hmem0 : (⊥ : Form L) ∈ Γ₀ := (Finset.mem_insert.mp hmem).resolve_left fun e => hhead e.symm
...
theorem Provable.removeFalsum {B : Ordinal.{0}} {Γ : Seq L}
    (h : Provable B 0 (insert (⊥ : Form L) Γ)) : Provable B 0 Γ := by
  rcases h with ⟨d, ho, hcr⟩
...
theorem Provable.cutElimPrincipal {c : ℕ} {ξ : Form L} {A B : Ordinal.{0}} {Γ : Seq L}
    (hξeq : ξ.complexity = c)
  ...
  | verum =>
      ...
      subst hc0
      have hNC' : Provable (Ordinal.omega0 ^ B) 0 (insert (⊥ : Form L) Γ) := hNC
...
theorem Provable.cutElimStepAux {c : ℕ} : ∀ {Γ : Seq L} (d : Deriv Γ), cr d ≤ ((c + 1 : ℕ) : ℕ∞) →
  ...
  | @axLv Γ k r v v' hval hp hn =>
    intro _; simp only [Deriv.o]
    exact (Provable.axLv r v v' hval hp hn).mono zero_le (Nat.zero_le c)
  | @axTrue Γ k b r v htrue hmem =>
    intro _; simp only [Deriv.o]
...
theorem Provable.cutElimStep {α : Ordinal.{0}} {c : ℕ} {Γ : Seq L}
...
theorem Provable.cutElim {α : Ordinal.{0}} {c : ℕ} {Γ : Seq L}
