...
theorem primrec_natLog : Primrec₂ (Nat.log) := by
  ...
  · -- correctness equation
    ...
    by_cases hn : n = 0
    · subst hn; simp only [Nat.log_zero_right, Nat.le_zero, List.length_nil]
      rw [if_neg (by omega)]
...
values. Lookups use `Option.iget` (`= ·.getD 0`) — on the genuine table both succeed. -/
theorem primrec_bump : Primrec₂ bump := by
  refine Primrec.nat_strong_rec bump (g := fun b L =>
      if L.length = 0 then some 0
        ...
        (b + 1) ^ ((L[Nat.log b L.length]?).iget) +
        (L[L.length % b ^ Nat.log b L.length]?).iget)) ?_ ?_
  · -- Primrec₂ g
    ...
    have pbe : Primrec (fun p : ℕ × List ℕ => (p.2[Nat.log p.1 p.2.length]?).iget) :=
      Primrec.option_iget.comp (Primrec.list_getElem?.comp Primrec.snd pe)
    ...
    have pbr : Primrec (fun p : ℕ × List ℕ =>
        (p.2[p.2.length % p.1 ^ Nat.log p.1 p.2.length]?).iget) :=
      Primrec.option_iget.comp (Primrec.list_getElem?.comp Primrec.snd pidx2)
    have pbase1 : Primrec (fun p : ℕ × List ℕ =>
        (p.1 + 1) ^ ((p.2[Nat.log p.1 p.2.length]?).iget)) :=
