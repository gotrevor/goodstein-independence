...
`REBUILD-Z-LAP1-VERDICT.md`.
- **THE single permitted advance still available** (calculus-independent growth lane, like laps 178–179; NOT gated
  because it discharges no §5 pin — it's a standalone `Hardy.lean` lemma the eventual P1 discharge will *consume*):
  prove the additive-Hardy **inequality** `hardy (e+β) x ≤ hardy e (hardy β x)` (NF `e,β`). Lap 178 refuted only the
  *equality*; the `≤` survives absorption and is P1's exact bridge from raised control `H_{e+ω^α}` to `H_e∘H_{ω^α}`,
  then (banked lap 179) `H_{ω^α}<f_α`. Non-absorbing = banked `hardy_add_comp`; full-absorption = `le_hardy`;
  real content = partial-absorption leading-term merge (decomposition in `PENDING_WORK.md`). If this lands or is
  refuted, that is the lap's crux advance; if it resists after honest attempts, record the obstruction and end.
- **Forbidden drift** (unchanged): manufacturing tractable side-leaves to look busy while the judge gate is the real
  blocker; grinding gated pins/Route-A; treating a green docs-only commit as the lap's advance.
- **Why this sticks:** lap 179's baton claimed the growth lane "comprehensively mined" — false; this names the one
  brick that remains, so the next grind lap has a concrete permitted target instead of idling on the gate.
