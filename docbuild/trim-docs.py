#!/usr/bin/env python3
"""Trim doc-gen4 output to just the GoodsteinPA library, linking dependencies out
to their hosted docs instead of shipping ~1.3 GB of them.

doc-gen4 has no native "document only my library" mode: the `:docs` facet renders
the entire import closure (all of Mathlib, Foundation, Std, Init, Lean, ...). For
this repo that is >99% bloat. This post-build step keeps only the GoodsteinPA
pages + the doc-gen4 chrome, deletes the dependency module trees, and rewrites
every cross-reference into a dependency so it points at the hosted docs:
Foundation modules -> https://formalizedformallogic.github.io/Foundation/docs/,
everything else    -> https://leanprover-community.github.io/mathlib4_docs/
(whose paths match doc-gen4's exactly). Result: a few MB, with upstream links
that still work. (Adapted from lean-gallery/docbuild/trim-docs.py.)

Caveat: the hosted docs track *current* mathlib / *upstream* Foundation, while
ours are pinned (and Foundation is the gotrevor v4.31 fork). Module-level paths
are very stable, so common links resolve; an occasional decl that has moved (or
is fork-only, e.g. InductionSchemeDelta1) may 404. That is the accepted cost of
linking out rather than self-hosting the whole closure.
...
# Dependency module trees with their own hosted docs (top-level module dir -> base URL).
HOSTED_FOR = {
    "Foundation": "https://formalizedformallogic.github.io/Foundation/docs/",
}
...
def build_link_rewriter(dep_mods):
    ...
    # trailing (/|.html) boundary already prevents `Lean` matching a longer sibling.
    alt = "|".join(re.escape(m) for m in sorted(dep_mods, key=len, reverse=True))
    pat = re.compile(
        r'((?:href|src|data-path)=")((?:\.\.?/)*)(%s)(?=/|\.html)' % alt
    )
    return lambda html: pat.sub(
        lambda m: m.group(1) + HOSTED_FOR.get(m.group(3), HOSTED) + m.group(3),
        html,
    )
